@component()
export class InputManager extends APJS.BasicScriptComponent {
    ...
    public oscillationSpeed: number = 120; // degrees per second during hold
    @serializeProperty



    public Do_it: boolean=false; // degrees per second during hold
    ...
    public returnSpeed: number = 600; // degrees per second on release (fast swing)
    ...
    public minAngle: number = 2.83; // initial/rest position
    ...
    public maxAngle: number = 275; // max backswing position
    ...
    public isWinnerReady: boolean = false;  // VS watches this — triggers when true
    ...
    public velocityMultiplier: number = 5.0; // hit and trial for best feel
    ...
    public cameraFollowSpeed: number = 3.0; // smooth follow speed
    ...
    public cameraZoomOutScale: number = 1.5; // how much camera zooms out
    ...
    public plankHideDelay: number = 7.0; // seconds before long plank 1 hides
    ...
    hitSoundPlayer!: APJS.SceneObject;  // drag audio SceneObject here
    ...
    private oscillationDirection: number = 1; // 1 = going to max, -1 = coming back
    ...
    private onTouchUp = (event: APJS.IEvent) => {
        ...
        if (!this.isPlaying || !this.isHolding) return;
        ...
        console.log("Released — club swinging fast! Saved Swing Angle: " + this.releaseAngle);
    ...
    onUpdate(deltaTime: number) {
        ...
        if (!this.initialized) {
            ...
            if (this.ballObject && this.cameraObject && this.golfBatObject) {
                ...
                this.ballRigidBody = this.ballObject.getComponent("RigidBody2D") as APJS.RigidBody2D;
                ...
                console.log("CombinedGameController: References Initialized!");
        ...
        // Hit audio init — separate from main init
        ...
        if (!this.hitAudioInitialized && this.hitSoundPlayer) {
            ...
            this.hitAudio = this.hitSoundPlayer.getComponent("AudioComponent") as APJS.AudioComponent;
        ...
        if (this.isBallHit && this.isFollowingBall && this.ballTransform && this.cameraTransform) {
    ...
    private handleBallHit(swingAngle: number): void {
        ...
        const normalizedAngle = totalRange > 0 ? (swingAngle - this.minAngle) / totalRange : 0;
        ...
        console.log("Ball hit via combined logic! Velocity applied: " + xVelocity);
    ...
    private hidePlank(): void {
        ...
        if (this.longPlank1Object) {
            ...
            const currentScene = this.getSceneObject().scene;
            ...
            if (currentScene) {
                ...
                currentScene.removeSceneObject(this.longPlank1Object);



                console.log("Long Plank 1 removed.");
    ...
    private calculateResult(): void {
        ...
        const screenTrans = this.ballObject.getComponent("ScreenTransform") as APJS.ScreenTransform;
    ...
    private getWinnerIndexAtX(x: number): number {
        ...
        for (let i = 0; i < this.vehicleRanges.length; i++) {
            ...
            if (x >= this.vehicleRanges[i][0] && x < this.vehicleRanges[i][1]) {
    ...
    private resetGameAndClub(): void {
        ...
        if (this.cameraTransform) {
            ...
            this.cameraTransform.localPosition = new APJS.Vector3f(-2.425, 0, 40);
